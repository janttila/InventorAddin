        ...
        Try
            ...
            If jsonValid AndAlso cfg IsNot Nothing Then
                ...
                For Each item In cfg
                    ' Log each config entry being registered
                    Try
                        DiagnosticsLogger.Log("Registering config: DisplayName='" & item.DisplayName & "' InternalId='" & item.InternalId & "' TabId='" & item.TabId & "' Handler='" & item.Handler & "'")
                    Catch
                    End Try
            ...
            End If
        Catch ex As Exception
            DiagnosticsLogger.Log("Error reading ButtonCfg.json: " & ex.Message)
        End Try
    Else
        DiagnosticsLogger.Log("ButtonCfg.json not found at: " & cfgPath)
    End If

    Try
        _rbManager.AddToUserInterface()
    Catch ex As Exception
        DiagnosticsLogger.Log("AddToUserInterface exception: " & ex.Message)
    End Try
End Sub
...
Private Function GetResourceObject(resourceName As String) As Object
    ...
    Catch ex As Exception
        DiagnosticsLogger.Log("GetResourceObject error for '" & resourceName & "': " & ex.Message)
        Return Nothing
    End Try
End Function
